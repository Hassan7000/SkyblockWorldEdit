package me.enchantbook.skyblockworldedit.Nbt.utils;
...
public class ApiMetricsLite {
	...
	public ApiMetricsLite() {
		...
		if (enabled) {
			...
			if(Bukkit.isPrimaryThread()){
				...
				if (!fFound) {
					SkyblockWorldEdit.getInstance().getLogger().info("[NBTAPI] Using the plugin '" + plugin.getName() + "' to create a bStats instance!");
					// We are the first!
					startSubmitting();
				}
			}else{
						...
						if (plugin instanceof JsonObject) {
							...
							try {
								...
								if (logFailedRequests) {
									SkyblockWorldEdit.getInstance().getLogger().log(Level.WARNING, "[NBTAPI][BSTATS] Encountered exception while posting request!", e);
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					...
					if (logFailedRequests) {
						SkyblockWorldEdit.getInstance().getLogger().log(Level.WARNING, "[NBTAPI][BSTATS] Could not submit plugin stats of " + plugin.getName(), e);
						// Not using the plugins logger since the plugin isn't the plugin containing the NBT-Api most of the time
	...
	private static void sendData(Plugin plugin, JsonObject data) throws Exception {
		...
		if (logResponseStatusText) {
			SkyblockWorldEdit.getInstance().getLogger().info("[NBTAPI][BSTATS] Sent data to bStats and received response: " + builder.toString());
			// Not using the plugins logger since the plugin isn't the plugin containing the NBT-Api most of the time
